...
public class BgpAllowAsInTest {
  ...
  public void testAllowasIn() throws IOException {
    ...
    if (SPINE.equals("spine")) {
      ...
      assert SPINE.equals("aspine");
      assertThat(
          backupRoutes.get(LEAF1, "default"),
          contains(
              allOf(
                  hasPrefix(PREFIX),
                  hasProtocol(RoutingProtocol.BGP),
                  hasAsPath(equalTo(AsPath.ofSingletonAsSets(1L, 2L))))));
    }
    ...
    if (SPINE.equals("spine")) {
      ...
      assert SPINE.equals("aspine");
      assertThat(
          routes.get(SPINE, "default"),
          contains(
              allOf(
                  hasPrefix(PREFIX),
                  hasProtocol(RoutingProtocol.BGP),
                  hasAsPath(equalTo(AsPath.ofSingletonAsSets(2L))))));
    }
